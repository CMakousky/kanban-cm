...
class AuthService {
  getProfile() {
    // TODO-COMPLETE: return the decoded token
    return jwtDecode<UserData>(this.getToken());
  }

  loggedIn() {
    // TODO-COMPLETE: return a value that indicates if the user is logged in
    const token = this.getToken();
    return !!token && !this.isTokenExpired(token);
  }
  
  isTokenExpired(token: string) {
    // TODO-COMPLETE: return a value that indicates if the token is expired
    try {
  ...
  getToken(): string {
    // TODO-COMPLETE: return the token
    const loggedUser = localStorage.getItem('id_token') || '';
    return loggedUser;
  }

  login(idToken: string) {
    // TODO-COMPLETE: set the token to localStorage
    localStorage.setItem('id_token', idToken);
    // TODO-COMPLETE: redirect to the home page
